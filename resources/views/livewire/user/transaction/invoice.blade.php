    ...
    <div class="container py-4">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                ...
                <p><strong>Metode Pembayaran:</strong> {{ $transaction->tst_payment_method }}</p>
                <p><strong>Total Pembayaran:</strong> Rp {{ number_format($transaction->tst_total, 0, ',', '.') }}</p>
                ...
                @foreach ($transaction->items as $item)
                    <li class="list-group-item d-flex justify-content-between">
                        <div>
                            ...
                            <small>{{ $item->tst_item_quantity }} ×
                                Rp {{ number_format($item->tst_item_price, 0, ',', '.') }}
                            </small>
                        </div>
                        <div>
                            <strong>Rp {{ number_format($item->tst_item_subtotal, 0, ',', '.') }}</strong>
                        </div>
                    </li>
                @endforeach
            </ul>
